...
*Resp.* As variáveis categóricas são nome; sexo; sobreviveu; classe; porto_embarque.
...
*Resp.* Mais mortes; 61% de mortes, 39% de sobreviventes 

### 3.1 Sabendo a quantidade exata

Como saber a quantidade exata de mortes nesse desastre?
Para isso, depois de selecionar a variável `sobreviveu`, faremos um filtro estabelecendo uma relação lógica com a resposta `"não"`.
...
*Resp.* Quantidade maior de mortes em relação ao sexo masculino; já que a corpo de uma mulher pode flutuar melhor na água, porque sua porcentagem de gordura corpórea é maior.
...
*Resp.* Os passageiros de classes mais altas conseguiram sair primeiro por ser mais proximo ao deque e alcançar um bote salva-vidas.
...
```
*Resp.*  Southampton teve o maior número de passageiros e por esse fato também teve o maior número de mortos.
